...
class ConsultController extends Controller
{
    ...
    public function reduceArr($consults)
    {
        ...
        foreach ($consults as &$consult) {
            ...
            list($consult['province'], $consult['city'], $consult['town']) =
                array_values(CallBackController::area($consult['province'] - 1, $consult['city'] - 1,
                    $consult['town'] - 1));
        }
    ...
    {
        if ( ! $consult = Consult::find($id)) {
            return ['code' => '1', 'msg' => '该次咨询不存在，请刷新后重试', 'time' => getNow()];
        }
    ...
    public function create()
    {
        ...
        if ($ways->isEmpty() || $diseases->isEmpty()) {
            return '错误：请至少添加一项途径，病种并且启用它';
        }
    ...
    public function store(ConsultRequest $req)
    {
        ...
        if ( ! Consult::create($data)) {
            return ['code' => '1', 'msg' => '咨询创建失败，请联系管理员', 'time' => getNow()];
        }
    ...
    {
        if ( ! $consult = Consult::find((int)$id)) {
            return;
        }
        $code = '1';
        $msg  = '已更新';
