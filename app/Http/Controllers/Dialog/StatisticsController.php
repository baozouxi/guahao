...
class StatisticsController extends Controller
{
    ...
    public function reduceArr($dialogs, $key)
    {
        ...
        foreach ($dialogs as $dialog) {
            ...
            if ( ! isset($data_arr[$field]['all_count'])) {
                $data_arr[$field]['all_count'] = 0;
            }
            if ( ! isset($data_arr[$field]['arrive'])) {
                $data_arr[$field]['arrive'] = 0;
            }
            if ( ! isset($data_arr[$field]['book_count'])) {
                $data_arr[$field]['book_count'] = 0;
            }
    ...
    public function searchByMonth(Request $req)
    {
        ...
        if ($req->has('key')) {
            return view('Dialog.Statis.listWithoutNav', ['total' => $total, 'dialogs' => $data_arr]);
        }

        return view('Dialog.Statis.search',
            ['total' => $total, 'dialogs' => $data_arr, 'months' => $months, 'current_month' => $current_month]);
    }
}
