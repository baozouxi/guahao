...
class DialogController extends Controller
{
    ...
    public function reduceArr($dialogs)
    {
        ...
        foreach ($temp_arr as $date => $item) {
            ...
            if ( ! isset($app_arr[$date]['arrive'])) {
                $app_arr[$date]['arrive'] = 0;
            }
            foreach ($item as $v) {
                if ($v['is_hospital'] == '1') {
                    $app_arr[$date]['arrive'] += 1;
                }
            }
        }
    ...
    public function create()
    {
        ...
        if ($ways->isEmpty()) {
            return '错误：缺少途径分类，请至少保留一条记录并启用它！';
        }
    ...
    public function store(DialogRequest $req)
    {
        $all          = $req->all();
        $data['date'] = $all['date'];
        $diaExist     = Dialog::where('date',
            formatDate($data['date'], 'Y-m-d'))->get([DB::raw("DATE_FORMAT(date, '%Y-%m-%d') as date")]);
        if ( ! $diaExist->isEmpty()) {
            return code('该日期已存在记录', '1');
        }
    ...
    public function update(DialogRequest $req, $id)
    {
        ...
        $data['data'] = serialize($all);
        if ( ! $dialog = Dialog::find($id)) {
            return ['code' => '1', 'msg' => '更新失败，请刷新后重试', 'time' => getNow()];
        }
        if ( ! $dialog->update($data)) {
            return ['code' => '1', 'msg' => '更新失败，请刷新后重试', 'time' => getNow()];
        }
    ...
    public function sheet()
    {
        ...
        return view('Dialog.sheet',
            ['dialogs' => $dialogs, 'ways' => $ways, 'total' => $total, 'months' => $months, 'users' => $users]);
    }
    ...
    public function sheetSearchByMonth(Request $req)
    {
        ...
        return view('Dialog.sheetSearch', [
            'dialogs'       => $dialogs,
            'ways'          => $ways,
            'total'         => $total,
