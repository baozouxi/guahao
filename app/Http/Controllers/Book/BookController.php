...
class BookController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($fields = Field::where('admin_id', '1')->where('type', '1')->first()) {
        ...
        return view('Book.index',
            ['data' => $appointData, 'count' => $count, 'fieldUrl' => $fieldUrl, 'fields' => $fields_list]);
    }
    ...
    public function today()
    {
        ...
        return view('Book.index',
            ['data' => $apps, 'count' => $count, 'fieldUrl' => $fieldUrl, 'fields' => $fields_list]);
    }
            ...
            foreach ($appointData as $num => $item) {
                ...
                switch ($k) {
                    ...
                    case 'name':
                        ...
                        if (check_node('patient_create')) {
                            $url = route('patient.create', ['bookId' => $item->id]);
                        }
                        if ($url) {
                            $item->$k = '<a href="javascript:void(0);" onclick="fastH(this,\'main\')" url="' . $url . '">' . $item->$k . '</a>';
                        }
                        if (check_node('book_info')) {
                            $info_html = 'title="“' . $name . '”的详细资料" onclick="msgbox(this,600);" url="' . $info_url . '" style="cursor:pointer;"';
                        }
                    ...
                    case 'file':
                        if ($item->chatlog) {
                            $data_arr[$num][$k] = '<td><center class="file"><a href="javascript:void(0);" title="“' . $name . '”的咨询记录" onclick="msgbox(this,850);" url="' . route('chatlog',
                                    ['id' => $item->id]) . '"><span class="icon">ĉ</span></a></center></td>';
                        }

                        if ($item->filepath) {
                            $data_arr[$num][$k] = '<td><center class="file"><a href="javascript:void(0);" title="“' . $name . '”的咨询记录" onclick="msgbox(this,350);" url="' . route('uploadPlay') . '?path=' . $item->filepath . '"><span class="icon">Ċ</span></a></center></td>';
                        }
                        break;
    ...
    public function create(Request $req)
    {
        ...
        if ($ways->isEmpty()) {
            $error = '错误：请保留只是一个预约途径选项并选择启用';
        }
        if ($diseases->isEmpty()) {
            $error = '错误：请保留只是一个病种选项并选择启用';
        }

        if ($error) {
            return view('Book.error', ['error' => $error]);
        }
