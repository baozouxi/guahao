...
class StatisticsController extends Controller
{
    ...
    public function reduceArr($data, $key)
    {
        ...
        foreach ($data as $field => $item) {
            ...
            if ( ! isset($data_arr[$field]['un_arrive'])) {
                $data_arr[$field]['un_arrive'] = 0;
            }
    ...
    public function searchByMonth(Request $req)
    {
        ...
        if (strtotime($req->month) && $req->month != 'm' && ! $req->has('key') || $req->month == '0') {
            ...
            return view('Book.Statistics.searchByMonth',
                ['total' => $total, 'data_arr' => $data_arr, 'months' => $months, 'current_month' => $req->month]);
        }
