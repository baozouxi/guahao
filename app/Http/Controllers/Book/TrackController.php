...
class TrackController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($data as &$dataItem) {
            ...
            foreach ($AppData as $appItem) {
                if ($dataItem->book_id == $appItem->id) {
                    $dataItem->name = $appItem->name;
                }
            }
        }
    ...
    public function show(Request $req, $id)
    {
        ...
        if ($req->m) {
            return view('book.track.show2', ['AppData' => $aModel, 'TrackData' => $TrackData]);
        }
    ...
    public function create(Request $req)
    {
        ...
        if ( ! $id = $req->session()->pull('track_book_id')) {
            return $error;
        }
    ...
    public function edit(Request $req, $id)
    {
        ...
        if ( ! $aModel = $this->getData($bookId)) {
            return ['code' => '1', 'msg' => '该预约不存在，请刷新后重试', 'time' => date('Y-m-d H:i:s')];
        }
        if ( ! $TrackData = Track::find($id)) {
            return ['code' => '1', 'msg' => '该回访不存在，请刷新后重试', 'time' => date('Y-m-d H:i:s')];
        }
    ...
    public function update(Request $req, $id)
    {
        ...
        if ( ! $res = $trackModel->save()) {
            return ['code' => '1', 'msg' => '更新失败，请稍后重试', 'time' => date('Y-m-d H:i:s')];
        }

        return [
            'code' => '0',
            'msg'  => route('booktrack.show', ['id' => $data['book_id']]),
    ...
    public function store(Request $req)
    {
        ...
        $data['admin_id']  = '1';
        if ( ! $model = Track::create($data)) {
            return ['code' => '1', 'msg' => '回访添加失败，请稍后重试', 'time' => date('Y-m-d H:i:s')];
        }
