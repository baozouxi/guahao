...
class PatientController extends Controller
{
    ...
    public function index(Request $req)
    {
        ...
        if ($fields = Field::where('admin_id', '1')->where('type', '2')->first()) {
    ...
    public function reduceArr($data)
    {
        ...
        $takes = Take::groupBy('patient_id')->select(DB::raw('sum(check_cost+treatment_cost+drug_cost+hospitalization_cost) as sum'),
            'patient_id')->get();
        $ids   = array();
        foreach ($data as &$item) {
            foreach ($takes as $take_item) {
                if ($take_item->patient_id == $item->id) {
                    $item->sum = $take_item->sum;
                }
            }
            $ids[] = $item->id;
            list($item->province, $item->city, $item->town) = array_values(CallBackController::area($item->province - 1,
                $item->city - 1, $item->town - 1));
        ...
        unset($doctors);
        $tracks = PatientTrack::whereIn('patient_id', $ids)->orderBy('next_time', 'desc')->get([
            'patient_id',
            'next_time'
        ])->toArray();
            ...
            foreach ($fields_list as $name => $item) {
                switch ($name) {
                    ...
                    case 'name':
                        ...
                        $info_html = '';
                        if (check_node('patient_info')) {
                            $info_html = 'title="“' . $name . '”的详细资料" onclick="msgbox(this,600);" url="' . route('patient.show',
                                    ['id' => $value->id]) . '" style="cursor:pointer;" class="icon"';
                        }
                        if (check_node('patient_edit')) {
                            $value->name = '<a href="javascript:void(0);" onclick="fastH(this,\'main\')" url="' . route('patient.edit',
                                    ['id' => $value->id]) . '">' . $value->name . '</a>';
                        }
                        $data_arr[$key][$name] = '<td><span ' . $info_html . '>Ĵ</span>' . $value->name . '</td>';
                        break;

                    case 'sum':
                        if ($value->sum == null || ! check_node('take_show')) {
                            $value->sum = 0;
                        }
                        if (check_node('take_edit')) {
                            $value->sum = '<a href="javascript:void(0);" onclick="fastH(this,\'main\')" url="' . route('takeWithInfo',
                                    ['patientId' => $value->id]) . '">' . $value->sum . '</a>';
                        } else {
                    ...
                    case 'track':
                        ...
                        if (count($value->tracks)) {
                            $track = '<u>' . formatDate($value->tracks['0']['next_time'],
                                    'm-d H:i') . '(' . count($value->tracks) . ')</u>';
                        }
                        if (check_node('track_edit')) {
                            $track = '<a href="javascript:void(0);" onclick="fastH(this,\'main\')" url="' . route('trackWithInfo',
                                    ['id' => $value->id]) . '">' . $track . '</a>';
                        }
    ...
    {
        if (isset($req->bookId)) {
            $book = Appointment::find((int)$req->bookId) ?: '';
        }
        ...
        $ads     = Ad::all();
        if ($dis->isEmpty()) {
            $error = '错误：请至少添加一个病种选项并且启用它';
        }
        if ($doctors->isEmpty()) {
            $error = '错误：请至少添加一个医生选项并且启用它';
        }

        if ($error) {
            return view('patient.error', ['error' => $error]);
        }

        if (isset($book) && $book !== '') {
            return view('patient.createWithBook', ['medical_num' => $medical_num, 'book' => $book]);
        }

        if ($ads->isEmpty()) {
            return view('patient.error', ['error' => '错误：请至少添加一个来源媒介选项并且启用它']);
        }

        return view('Patient.create',
            ['medical_num' => $medical_num, 'diseases' => $dis, 'doctors' => $doctors, 'ads' => $ads]);
    }

    public function store(PatientStoreRequest $req)
    {

        if ( ! $req->session()->has('medical_num')) {
            return ['code' => '1', 'msg' => '请勿重复提交', 'time' => date('Y-m-d H:i:s')];
        }
        ...
        try {
            ...
            if (isset($data['book_id']) && $data['book_id'] !== '0') {
                ...
                $book->is_hospital = '1';
                if ( ! $book->save()) {
                    throw new \Exception("预约修改失败");
                }
            }

            if ( ! $data = Patient::create($data)) {
                throw new \Exception('病人添加失败');
            }
    ...
    public function edit(Request $req, $id)
    {
        ...
        $medical_num = mt_rand(100, 999) . time();
        $req->session()->put('medical_num', $medical_num);
        if ( ! $data = Patient::find($id)) {
            return ['code' => '1', 'msg' => '该患者不存在', 'time' => date('Y-m-d H:i:s')];
        }
    ...
    public function update(PatientStoreRequest $req, $id)
    {
        ...
        try {
            ...
            if ( ! $data->update($infoData)) {
                throw new \Exception('更新失败，请稍后重试');
            }

            if ($data->book_id !== 0) {
                if ( ! $book = Appointment::find($data->book_id)) {
                    throw new \Exception("预约修改失败，请稍后重试");
                }
                if ( ! $book->update($infoData)) {
                    throw new \Exception('预约信息修改失败，请联系管理员');
                }
            }
            DB::commit();
    ...
    public function patientReport(Request $req, $date = '', $doctor = '0')
    {
        ...
        if ($req->doctor) {
            $patients->where('doctor', (int)$req->doctor);
        }
        ...
        foreach ($patients as $item) {
            ...
            if ( ! isset($sum_item_arr[$date_index])) {
                $sum_item_arr[$date_index] = 0;
            }
            $sum_item_arr[$date_index] += 1;
            if ( ! isset($data[$date_index]['web'])) {
                $data[$date_index]['web'] = null;
            }
            if ( ! isset($data[$date_index]['another'])) {
                $data[$date_index]['another'] = null;
            }
        }
    ...
    public function statistics(Request $req)
    {
        ...
        if ($req->has('key')) {
            return view('Patient.statistics.listWithoutNav', ['all_count' => $count, 'data' => $patients]);
        }
    ...
    public function staReduceArr($patients, $key = 'admin_id')
    {
        ...
        foreach ($patients as &$patient) {
            ...
            list($patient['province'], $patient['city'], $patient['town']) =
                array_values(CallBackController::area($patient['province'] - 1, $patient['city'] - 1,
                    $patient['town'] - 1));
        }
    ...
    public function statisByMonth(Request $req)
    {
        ...
        $patients   = $this->staReduceArr($patients, $key);
        if ($req->has('key')) {
            return view('Patient.statistics.listWithoutNav', [
                'all_count'     => $count,
                'data'          => $patients,
                'current_month' => $month,
                'months'        => $months
        ...
        return view('Patient.statistics.listSearchByMonth', [
            'all_count'     => $count,
            'data'          => $patients,
            'current_month' => $month,
            'months'        => $months
    ...
    public function timeToTrack()
    {
        ...
        $patient_id_arr = [];
        $tracks         = PatientTrack::groupBy('patient_id')->get([
            DB::raw('max(next_time) as next_time'),
            'patient_id'
        ]);
