...
class StatisticsController extends Controller
{
    ...
    private function reduceArr($records, $key)
    {
        ...
        foreach ($records as $item) {
            ...
            if ( ! isset($data_arr[$field]['cost'])) {
                $data_arr[$field]['cost'] = 0;
            }
            if ( ! isset($data_arr[$field]['click'])) {
                $data_arr[$field]['click'] = 0;
            }
            if ( ! isset($data_arr[$field]['show_times'])) {
                $data_arr[$field]['show_times'] = 0;
            }
    ...
    public function searchByMonth(Request $req)
    {
        ...
        if ($req->has('key')) {
            return view('Rank.Statis.listWithoutNav', ['data_arr' => $data_arr, 'total' => $total]);
        }

        return view('Rank.Statis.search',
            ['data_arr' => $data_arr, 'total' => $total, 'months' => $months, 'current_month' => $current_month]);
    }
}
