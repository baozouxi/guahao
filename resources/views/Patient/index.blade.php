...
<div class="guide">
    ...
    <p class="nlink right">
        ...
        <a
                href="javascript:void(0);" onclick="fastH(this, 'main');set_title('到期回访');"
                url="{{ route('patientNeedTrack') }}" class="sms"><span class="icon">ĝ</span>到期回访</a><a
                href="javascript:void(0);" onclick="fastH(this,'main');set_title('今日到诊');" url="/patient/come/today"
                class="sms"><span class="icon">Ğ</span>今日到诊</a></p>
</div>
    ...
    <div id="container" class="container">
        <div class="top">
            ...
            <p class="nlink right"><a href="javascript:void(0);" onclick="fundisp()"><span class="icon">Ş</span>切换</a><a
                        href="javascript:void(0);" title="显示表格" onclick="msgbox(this,600);" url="{{ $fieldUrl }}"
                        class="config"><span class="icon">Ƅ</span>设置</a>
                @if(check_node('patient_export'))
                    <a href="javascript:void(0);" title="导出电子表格" onclick="msgbox(this);"
                       url="{{ route('patientExportHtml') }}" class="excel"><span class="icon">Ľ</span>导出</a>
                @endif
            </p>
        </div>
        <div class="fun">
            @if(check_node('patient_add'))
                <h3 class="left"><a href="javascript:void(0);" onclick="fastH(this,'main')"
                                    url="{{ route('patient.create') }}"><span class="icon">ŷ</span>新增患者</a></h3>
            @endif
                ...
                <form name="form_date" id="form_date" onsubmit="return(fastK(this,'ds'));" action="/patient/month/">
                    <input name="ds" id="ds" class="inp" type="text" value=""
                           onfocus="WdatePicker({onpicked:function(){de.focus();},maxDate:'#F{$dp.$D(\'de\')||\'%y-%M-%d\'}'})"><i
                            class="calendar icon">ğ</i>
                    <input name="de" id="de" class="inp" value=""
            ...
            <table cellspacing="1" cellpadding="0">
                ...
                <tr>
                    @foreach($fields_list as $list)
                        <th width="{{ $list['width'] }}">
                            <center><a href="javascript:void(0);" url="turn.asp?t=id&amp;go=desc&amp;m=turn"
                                       title="按编号排序" onclick="fastH(this)">{{ $list['name'] }}</a></center>
                        </th>
                @endforeach
                <!--                             <th width="120">
